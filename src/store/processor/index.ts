...
Object.keys(storeModule).forEach(value => {
  ...
  const effect = Object.keys(storeModule[value].effect).reduce((effectArr: Array<any>, funcName: string): Array<any> => {
  ...
  reducer[value] = function (state: any = initState, action: any) {
